...
func (d *Dispatcher) verifyAndRevise(ctx context.Context, req GenerateRequest, bundle ContextBundle, content string, pl PipelineName, maxIter int, emit func(ProgressEvent), outline string) (string, error) {
	...
	if req.TargetWord > 0 {
		if cur := []rune(current); len(cur) > req.TargetWord*13/10 {
			cut := trimToSentenceBoundary(current, req.TargetWord*11/10)
	...
	lengthOK := func(text string) bool {
		...
		return len([]rune(text)) <= req.TargetWord*13/10
	}
	...
	for iter := 1; iter <= maxIter; iter++ {
		...
		if req.TargetWord > 0 {
			if curLen := len([]rune(current)); curLen > req.TargetWord*13/10 {
				cut := trimToSentenceBoundary(current, req.TargetWord*11/10)
