...
func (s *Store) CreateStyleSample(ctx context.Context, m StyleSample) (*StyleSample, error) {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO style_samples(id, title, author, category, source_file, content, created_at)
		VALUES(?,?,?,?,?,?,?)`,
		m.ID, m.Title, m.Author, m.Category, m.SourceFile, m.Content, m.CreatedAt)
	if err != nil {
		return nil, err
	}
	return &m, nil
...
func (s *Store) ListStyleSamples(ctx context.Context, category string) ([]StyleSample, error) {
	q := `SELECT id, title, author, category, source_file, content, created_at FROM style_samples`
	args := []interface{}{}
	if category != "" {
		q += ` WHERE category=?`
	...
	for rows.Next() {
		var m StyleSample
		if err := rows.Scan(&m.ID, &m.Title, &m.Author, &m.Category, &m.SourceFile, &m.Content, &m.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, m)
	}
...
func (s *Store) ListStyleSampleMeta(ctx context.Context) ([]StyleSample, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, title, author, category, source_file, '', created_at FROM style_samples ORDER BY title, created_at DESC LIMIT 2000`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetStyleSample(ctx context.Context, id string) (*StyleSample, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT id, title, author, category, source_file, content, created_at FROM style_samples WHERE id=?`, id).
		Scan(&m.ID, &m.Title, &m.Author, &m.Category, &m.SourceFile, &m.Content, &m.CreatedAt)
...
func (s *Store) GetStyleSamplesByIDs(ctx context.Context, ids []string) ([]StyleSample, error) {
	...
	q := `SELECT id, title, author, category, source_file, content, created_at FROM style_samples WHERE id IN (`
...
func (s *Store) ReplaceStyleSamples(ctx context.Context, samples []StyleSample) (int, error) {
	...
	for _, m := range samples {
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO style_samples(id, title, author, category, source_file, content, created_at)
			VALUES(?,?,?,?,?,?,?)`,
			m.ID, m.Title, m.Author, m.Category, m.SourceFile, m.Content, m.CreatedAt); err != nil {
			return count, err
		}
		count++
	}
